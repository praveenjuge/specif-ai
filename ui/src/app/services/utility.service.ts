...
export class UtilityService {
  ...
  getProductRequirements(
    ...
    if (screensText && screensText.trim() !== productRequirements.trim()) {
      details += `<br><br><h3 class="${fontWeight} text-slate-900 hover:text-slate-600">SCREENS</h3>${screensText.trim()}`;
    }

    if (personasText && personasText.trim()) {
      details += `<br><br><h3 class="${fontWeight} text-slate-900 hover:text-slate-600">PERSONAS</h3>${personasText.trim()}`;
    }
